...
pubkey = virtualchain.lib.ecdsalib.ecdsa_private_key(privkey).public_key().to_hex() # keylib.ECPrivateKey(privkey, compressed=False).public_key().to_hex()
...
def mktx(satoshis, fee):
    ...
    if satoshis is None:
        ...
        print 'WARN: sending {} from {} to {}'.format(satoshis, payment_addr, recipient_addr)
